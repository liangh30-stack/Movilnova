...
const PromoBannerCarousel: React.FC<PromoBannerCarouselProps> = ({ shopBannerEnabled, shopBannerText, shopBannerSubtext }) => {
  ...
  return (
    <div className={`w-full bg-gradient-to-r ${current.gradient} transition-all duration-700`}>
      <div
      ...
      >
        ...
        {slides.length > 1 && (
          <div className="absolute right-4 flex gap-1.5 items-center">
            {slides.map((_, i) => (
              <button
                key={i}
...
const Storefront: React.FC<StorefrontProps> = ({ products, onAddToCart, lang, onTrackOrderClick, favorites, onToggleFavorite, featuredProductId, bannerEnabled, bannerText, bannerSubtext, showOffersOnly, onClearOffersFilter }) => {
  ...
  return (
    <div className="bg-brand-light min-h-screen pb-20 font-sans">
      ...
      <PromoBannerCarousel
        shopBannerEnabled={bannerEnabled}
        shopBannerText={bannerText}
        shopBannerSubtext={bannerSubtext}
      />
